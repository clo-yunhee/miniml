...
#define apply(expr)   infer_apply(subs, expr)
#define applyl(list)  apply_list(subs, list)
...
Type *apply_one(SubstList *subs, Type *type) {
    ...
    return subst_sub(first, apply_one(next, type));
}
...
TypedAst *infer_apply(SubstList *subs, TypedAst *expr) {
    ...
    expr->xtype = apply_one(subs, expr->xtype);
