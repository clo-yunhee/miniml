...
Les structure de type et de valeur sont implémentées de la même façon que les AST.
Le tableau suivant décrit les relations entre les types et les valeurs.
...
\hline
    ...
    unit     & - & -             \\ \hline
    int      & - & Un entier        \\ \hline
    float    & - & Un flottant      \\ \hline
    bool     & - & Un booléen       \\ \hline
    string   & - & Une chaîne de caractères     \\ \hline
        native function 1   & \multirow{2}{*}{La liste des arg}La liste des & name: Int         \\ \hline
    native function 2          & expr: AST         \\ \hline
    declared function            & list: List[AST]   \\ \hline
    tuple     & function: AST,
    polymorphic                    args: List[AST]                 \\ \hline
    error             & names: List[AST],
                        rec: Bool, params: List[AST],
                        expr: AST, block: AST           \\
\hline
\end{tabular}
...
Le compilateur utilise le système de typage Hindley-Milner pour inférer le type d'une expression.
